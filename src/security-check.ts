...
const IGNORED_DIRECTORIES = new Set([
  ...
  "node_modules", "bower_components", "dist", "build", ".next", "coverage", "htmlcov", ".nyc_output",
  // Test fixtures and harnesses are not production attack surfaces by default.
  "test", "tests", "e2e",
  // Python
...
function scanGeneralVulnerabilities(filePath: string, content: string, language: SecurityLanguage): SecurityFinding[] {
  ...
  const containsFixedSqlIdentifierList = /\b(?:allowed|permitted|valid|safe)[_-]?(?:table|column|identifier)s?\b/i.test(content) || /(?:^|\n)\s*tables?\s*=\s*\[/i.test(content);
  lines.forEach((line, index) => {
    ...
    if (language === "python") {
      if (/(?:subprocess\.(?:run|call|Popen)|os\.system|os\.popen)\s*\(/.test(line) && ( /shell\s*=\s*True/i.test(line) || /(?:f["']|["'][^"']*["']\s*\+\s*[A-Za-z_]|[A-Za-z_]\w*\s*\+\s*["'])/.test(line))) add("general-command-injection", "command", "CWE-78", "A03:2021-Injection", "外部入力がコマンド実行APIへ到達する可能性があります。", "shell実行を避け、固定コマンドと引数配列を使用してください。");
      if (/(?:cursor|connection|db|session)\.(?:execute|query)\s*\(/.test(line) && /(?:f["']|\+|%|format\s*\()/.test(line)) add("general-sqli", "sql", "CWE-89", "A03:2021-Injection", "動的なSQL文字列を実行している可能性があります。", "parameterized queryを使用してください。", containsFixedSqlIdentifierList ? "low" : "medium");
      ...
      if (/(?:requests|httpx|aiohttp)\.(?:get|post|request)|urllib\.request\.urlopen\s*\(/.test(line) && /(?:request|url|target|input)/i.test(line)) add("general-ssrf", "http-client", "CWE-918", "A10:2021-SSRF", "外部入力でHTTP接続先を指定できる可能性があります。", "接続先を許可リストで制限してください。");
