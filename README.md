...
```bash
npm install -g ./agent-safecheck-0.1.0.tgz
```
...
link後は、ローカル環境で `safecheck` コマンドを使えます。
...
```bash
safecheck security-check --json > safecheck-security.json
```
...
```bash
safecheck security-check --sarif > safecheck.sarif
```
...
- [GitHub Actions Secret allowlist実装計画](docs/implementation/workflow-secret-allowlist-plan.md)
